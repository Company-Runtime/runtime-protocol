...
| `pnpm run lint:semantic`     | The semantic constitution's lint rules `SL001`–`SL020` over the registry and `extensions/`                              |
| `pnpm run check:compat`      | Compatibility against a base ref (`COMPAT_BASE`, default `origin/main`): removals, version bumps, status regressions    |
| `pnpm run validate:bindings` | HTTP OpenAPI references, MCP tool-name reversibility, lossless CloudEvents mapping, binding examples                    |
| `pnpm run docs:check`        | Links and anchors; spec tables equal the schemas, registry and lint rules                                               |
...
Similarity findings (`SL008`, `SL010`) are warnings for human review; accepted ones are
recorded in `tooling/data/lint-acknowledgments.yaml` with a link to the decision.
Vendor and transport term lists live in `tooling/data/`.
